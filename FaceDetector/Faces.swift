...
extension UIImage {
    func detectFaces(completion: @escaping ([VNFaceObservation]?) -> ()) {
        ...
        DispatchQueue.global().async {
            ...
            guard let observation = request.results as? [VNFaceObservation]
    ...
    func rotatedBy(degree: CGFloat, clockwise: Bool = false) -> UIImage? {
    ...
    func drawnOn(_ image: UIImage) -> UIImage? {
        ...
        for observation in self {
            ...
            if var overlayImage = randomEmoji.image(of: paddedOverlayRect.size) {
                ...
                if let angle = anchor.angle, let rotatedImage = overlayImage.rotatedBy(degree: angle) {
